...
```{r}
sequenceNums <- c(11, 6, 8, 11, 12, 11, 3, 7, 10, 8)
set<-(sequenceNums%%2)==0
pick_evens<-sequenceNums[set]
pick_evens
```
...
set<-c("法學院","商學院")
pick_lawBusiness<-transcript100to103$學院%in%set
pick_lawBusiness
```
...
print(Date_reformat)
lubridate::ymd(Date_reformat)->Date_reformat90
class(Date_reformat)
```

```{r}
char4 <- c("Wild","Apple","Donald","May") 
factor(char4)

char3 <- c("蔡英文","習近平","Donald Trump") -> G3
factor(char3)
```
...
factor1
levels(factor1) <- c("北部","北部","北部","中部","北部","中部")
factor1
```
